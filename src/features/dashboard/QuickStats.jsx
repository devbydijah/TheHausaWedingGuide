...
export default function QuickStats({ data }) {
  ...
  const budgetCompletion = budgetTotal > 0 ? (budgetAllocated / budgetTotal) * 100 : 0;

  const totalVendors = data.vendorList?.length || 0;
  const bookedVendors = data.vendorList?.filter((v) => v.status === "Booked").length || 0;
  const vendorCompletion = totalVendors > 0 ? (bookedVendors / totalVendors) * 100 : 0;

  const totalTasks = data.taskList?.length || 0;
  const completedTasks = data.taskList?.filter((t) => t.status === "Completed").length || 0;
  const taskCompletion = totalTasks > 0 ? (completedTasks / totalTasks) * 100 : 0;

  const today = new Date();
  const overdueTasks = data.taskList?.filter((task) => {
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
      {stats.map((stat, index) => (
        <Card key={index} className="p-6">
          ...
          <div className="text-3xl font-bold text-gray-900 dark:text-white mb-2" style={{ color: stat.color }}>
            {stat.value}
          </div>
          {stat.subtitle && (
