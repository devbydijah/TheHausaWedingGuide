...
export default function MobileNav({ isOpen, onClose, sections, activeSection, onSectionChange }) {
  return (
    <div className={`fixed inset-0 z-50 ${isOpen ? 'block' : 'hidden'} md:hidden`}>
      ...
      <div className="absolute top-0 right-0 h-full w-64 bg-white shadow-2xl p-6">
        <button onClick={onClose} className="mb-8">✕ Close</button>
        <nav>
          {sections.map(section => (
            <button
              key={section.id}
              onClick={() => { onSectionChange(section.id); onClose(); }}
              className={activeSection === section.id ? 'bg-[#CE805C] text-white' : 'text-gray-900'}
            >
              {section.label}
            </button>
          ))}
