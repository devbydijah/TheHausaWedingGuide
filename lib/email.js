 ...
 */
export async function sendWebAppAccessEmail(email) {
  try {
    const webAppLink = `${WEB_APP_URL}/?guide=1&email=${encodeURIComponent(email)}`;
 ...
 */
export async function sendBundleEmail(email, downloadLink) {
  try {
    const webAppLink = `${WEB_APP_URL}/?guide=1&email=${encodeURIComponent(email)}`;
