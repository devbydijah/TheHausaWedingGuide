...
export async function sendDownloadEmail(email, downloadLink) {
  try {
    ...
    console.log(
      "[EMAIL] PDF download email sent to:",
      email.replace(/(.{2}).*(@.*)/, "$1***$2")
    );
    return data;
  } catch (error) {
...
export async function sendWebAppAccessEmail(email, txReference) {
  console.log(
    `[EMAIL] 📧 Attempting to send web app email to: ${email.replace(/(.{2}).*(@.*)/, "$1***$2")}`
  );
  console.log(
    `[EMAIL] 🔑 RESEND_API_KEY exists: ${!!process.env.RESEND_API_KEY}`
  );
...
Transaction Reference: ${txReference}`,
    ...
    console.log(
      `[EMAIL] ✅ Web app email sent successfully! Message ID: ${data.id}`
    );
    console.log(
