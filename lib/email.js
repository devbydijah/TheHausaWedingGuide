...
export async function sendDownloadEmail(email, downloadLink) {
  try {
    ...
    console.log('[EMAIL] PDF download email sent to:', email.replace(/(.{2}).*(@.*)/, '$1***$2'));
    return data;
  } catch (error) {
    console.error('[EMAIL] Failed to send PDF email:', error);
    throw error;
  }
}
...
Transaction Reference: ${txRef}`
    ...
    console.log('[EMAIL] Web app email sent to:', email.replace(/(.{2}).*(@.*)/, '$1***$2'));
    return data;
  } catch (error) {
    console.error('[EMAIL] Failed to send web app email:', error);
    throw error;
  }
}
 ...
 */
export async function sendBundleEmail(email, downloadLink, txRef, hasExistingAccount = false) {
  try {
    const signupUrl = `${WEB_APP_URL}/?guide=1&email=${encodeURIComponent(email)}`;
    ...
    const { data, error } = await resend.emails.send({
      from: FROM_ADDRESS,
      to: [email],
      subject: 'You Now Have Both Products - Complete Wedding Planning Package!',
      html: `
...
Transaction Reference: ${txRef}`
    ...
    console.log('[EMAIL] Bundle email sent to:', email.replace(/(.{2}).*(@.*)/, '$1***$2'));
    return data;
  } catch (error) {
